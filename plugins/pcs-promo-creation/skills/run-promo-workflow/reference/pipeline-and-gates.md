...
|---|-------|----------------------------------------|--------|
| 0 | Step 0, only if `kb` must be installed/upgraded | `kb <version?> needs installing/upgrading to >= 0.5.17. Run the pip install now? (Y/N)` | Stop; show manual install steps and exit. |
