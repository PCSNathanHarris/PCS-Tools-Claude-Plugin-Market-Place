...
**18 columns** (v1.2.0), single row per parse run. **This file is ALWAYS
...
```csv
Deck,Vendor,Total Pages,Kit Pages,NLP Pages,RSA Pages,Excluded Pages,Other-Promotions Pages,Promo Rows,NLP Rows,RSA Kit Rows,RSA NLP Rows,Needs Pricing Rows,Non-Included Count,Other-Promotions Rows,For-Review Rows,Run At UTC,Flags
Milwaukee_Q2_2026.pdf,Milwaukee,98,28,55,5,5,3,412,287,18,7,0,12,9,4,2026-06-18T17:23:00Z,crescent-unconfirmed
```
...
`low-confidence`) plus any row missing required data (Review Class
`missing-data`). Generated by a throwaway `openpyxl` script (the marketplace
...
| # | Column | Meaning |
|---|--------|---------|
| 1 | `Review Class` | `low-confidence` \| `missing-data` |
...
| 8 | `Missing Field` | `price` \| `sku` for missing-data; blank for low-confidence |
...
**Surfacing rules (For-Review is additive — it never re-routes a row):**
